...
/**
 * Extract an array of buffers from a string, parsing multiple buffer names and
 * channel keys
...
export function parseIrcUri(str) {
  ...
  str.split(';').forEach((connectionString) => {
    ...
    channels = _(channels.split(',')).compact().map((_channelName) => {
...
/**
 * Replace the target object with source, while keeping the target object
 * reference intact. Delete all the properties from the target object and copy
 * the properties from source over to the target. a = {one: 1, two: 2, three: 3}
 * b = {four: 4, five: 5, six: 6}
 * replaceObjectProps(a, b)
 * a.one === undefined;
 * a.six === 6;
 * @param {Object} target The target object that will be replaced
 * @param {Object} source The source object from which all properties will be
 *     copied from
 */
 ...
 * @param {Date} date The date object to create the time from. Defaults to the
 *     current time
 */
