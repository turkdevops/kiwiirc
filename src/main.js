...
// Add the global API as soon as possible so that things can start listening to
// it
...
// Add a handy this.listen() fn to Vue instances. Saves on the need to add an
// event listener and then manually remove them all the time.
Vue.mixin({
  beforeDestroy : function
      beforeDestroy() { (this.listeningEvents || []).forEach((fn) => fn()); },
  methods : {
    listen : function listen(source, event, fn) {
      this.listeningEvents = this.listeningEvents || [];
      this.listeningEvents.push(() => {
        (source.removeEventListener || source.$off || source.off)
            .call(source, event, fn);
      });
      (source.addEventListener || source.$on || source.on)
          .call(source, event, fn);
    },
  ...
  bind(el, binding) {
    if (binding.value.nodeName) {
      ...
      if (binding.value.data) {
        Object.keys(binding.value.data)
            .forEach(
                (key) => { rawEl.dataset[key] = binding.value.data[key]; });
      }

      // Add any properties to the raw element
      if (binding.value.props) {
        Object.keys(binding.value.props)
            .forEach((key) => { rawEl[key] = binding.value.props[key]; });
      }
    }
  },
});
...
function loadApp() {
   ...
   *   1. If a 'config' argument is in the query string,
   * static/config_THEVALUE_.json
   *   2. If a window.kiwiConfig function exists, use it's return value as the
   * config object.
   *   3. If a <meta name="kiwiconfig" content=""> is found, then the content
   * becomes the config URL.
   *   4. If a <script name="kiwiconfig" type="application/json"></script> is
   * found, then the content becomes the config JSON without making another web
   * request.
   */
  if (getQueryVariable('config')) {
    ...
    let configContents =
        document.querySelector('script[name="kiwiconfig"]').innerHTML;
  ...
  configLoader.addValueReplacement('protocol', window.location.protocol)
  ...
  (configObj ? configLoader.loadFromObj(configObj)
  ...
  return new Promise((resolve, reject) => {
    ...
    function loadNextScript(sSrc, fOnload) {
      ...
      if (plugin.url.indexOf('.js') > -1) {
        ...
        scr.onerror = () => {
          log.error(
              `Error loading plugin '${plugin.name}' from '${plugin.url}'`);
          loadNextScript();
        };
        scr.onload = () => { loadNextScript(); };

        document.body.appendChild(scr);
        scr.src = plugin.url;
      } else {
        // Treat the plugin as a HTML document and just inject it into the
        // document
        fetch(plugin.url)
            ...
            .then((pluginRaw) => {
              ...
              // The browser won't execute any script elements so we need to
              // extract them and place them into the DOM using our own script
              // elements
...
function initLocales() {
  ...
  const setDefaultLanguage = () => {
    ...
    let preferredLangs =
        _.clone(window.navigator && window.navigator.languages) || [];
    ...
    for (let idx = 0; idx < preferredLangs.length; idx++) {
      ...
      // if this is a language such as 'fr', add a following one of 'fr-fr' to
      // cover both cases
      if (lang.length === 2) {
...
async function initState() {
  ...
  let persist =
      new StatePersistence(stateKey || '', state, Storage, persistLog);
