    ...
    methods: {
        listen: function listen(source, event, fn) {
            ...
            this.listeningEvents.push(() => {
                (source.removeEventListener || source.$off || source.off).call(
                    source,
                    event,
                    fn
                );
            });
            (source.addEventListener || source.$on || source.on).call(
                source,
                event,
                fn
            );
...
function loadApp() {
    ...
    if (getQueryVariable("config")) {
        ...
        let configContents = document.querySelector('script[name="kiwiconfig"]')
            .innerHTML;
            ...
            if (plugin.url.indexOf(".js") > -1) {
                ...
                fetch(plugin.url)
                    ...
                    .then((pluginRaw) => {
                        ...
                        el.id =
                            "kiwi_plugin_" + plugin.name.replace(/[ "']/g, "");
    ...
    state.$watch("user_settings.language", (lang) => {
        if (!lang && !state.setting("language")) {
            ...
            i18next.changeLanguage(
                lang || state.setting("language") || "en-us"
            );
        }
    });
}
