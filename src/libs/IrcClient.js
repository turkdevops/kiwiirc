  ...
  function parsedEventsHandler(command, event, client, next) {
    ...
    if (command === 'registered') {
      ...
      state.addUser(networkid,
                    {nick : event.nick, username : client.user.username});
      ...
      state.addMessage(serverBuffer, {
        ...
        nick : '',
        message :
            TextFormatting.t('connected_to', {network : client.network.name}),
      });
    ...
    if (command === 'account') {
      state.addUser(networkid,
                    {nick : event.nick, account : event.account || ''});
    }
        ...
        event.modes.forEach((mode) => {
          ...
          if (prefix) {
            ...
            modeStrs[mode.mode].push(
                {target : buffer.name, param : mode.param});
          }
        });
