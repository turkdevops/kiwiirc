...
export default class BouncerProvider {
    constructor(state) {
        ...
        // A snapshot of the current networks. Compared against to detect changed
        // networks
    ...
    getController() {
        if (
            this.controllerNetwork &&
            this.controllerNetwork.state === "connected"
        ) {
        ...
        for (let i = 0; i < this.state.networks.length; i++) {
            ...
            if (
                ...
                client.network.cap.isEnabled("bouncer")
            ) {
    ...
    async onNetworkMotd(event, network) {
        ...
        // Set the bncnetid if the network upstream exists and we havn't already set
        // it
        ...
        // If this is a BNC network, sync it before anything else so that we get all
        // it's info and buffer states as soon as possible
    ...
    onNetworkState(event, network) {
        ...
        // We get connection state changes advertised on all bouncer connections, so
        // only handle the ones that come from the controller network since we only
        // need to deal with it once.
        if (isController && event.state === "disconnected") {
            // Mimick any network disconnections by closing this connection too
            let effectedNetwork = this.state.getNetworkFromBncNetId(
                event.networkId
            );
            if (effectedNetwork) {
    ...
    async syncBncNetwork(bncNetwork) {
        ...
        let buffers = await client.bnc.getBuffers(
            bncNetwork.connection.bncnetid
        );
        ...
        bncNetwork.buffers.forEach((clientBuffer) => {
            ...
            let existingBuffers = buffers.filter(
                (bncBuffer) =>
                    bncBuffer.name.toLowerCase() ===
                    clientBuffer.name.toLowerCase()
            );
    ...
    saveState() {
        ...
        this.state.networks.forEach((network) => {
            ...
            if (
                this.getController() === network &&
                !network.ircClient.bnc.hasNetwork()
            ) {
                return;
            }

            // Don't save any hidden networks. This includes the BNC controller
            // network
            if (network.hidden) {
                return;
            }
            ...
            // A newly added network would not have a snapshot name (bncnetid)
            // property set yet. Only save the network if we've entered connection
            // info.
            if (!snapshot.bncnetid && tags.host && tags.port && tags.nick) {
                // ?? network.connection.bncname = network.name;
                controller.ircClient.bnc
                    .addNetwork(
                        network.name,
                        tags.host,
                        tags.port,
    ...
    listenToState() {
        ...
        // Just before we connect to a network, make sure the BNC is saved and
        // connected to it or at least trying to connect. Ie. Quickly creating a
        // network and hitting connect before it's had time to
        //     save itself to the bouncer
        state.$on("network.connecting", (event) => {
            // Redirect the connection towards the bouncer with the network specific
            // password
            ...
            if (this.bnc.enabled && this.rewriteConnections) {
                ...
                // The SASL auth already happens on the BNC, we only use it for UI
                // purposes in kiwi
        ...
        state.$on("network.connecting", (event) => {
            ...
            if (network.connection.bncnetid) {
                controller.ircClient.raw(
                    "BOUNCER connect " + network.connection.bncnetid
                );
            }
        });
        ...
        state.$on("network.new", (event) => {
            ...
            // Update the network name to NetworkN if hasn't got once from the bouncer
            // yet
            if (!network.connection.bncnetid) {
                let currentNum = 1;
        ...
        state.$on("network.removed", (event) => {
            ...
            if (event.network.connection.bncnetid) {
                controller.ircClient.bnc.removeNetwork(
                    event.network.connection.bncnetid
                );
            }
        });
