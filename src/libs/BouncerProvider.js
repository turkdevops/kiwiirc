...
export default class BouncerProvider {
  ...
  async syncBncNetwork(bncNetwork) {
    ...
    buffers.forEach((buffer) => {
      ...
      if (bncNetwork.state === "connected" && newBuffer.isChannel() &&
          newBuffer.joined) {
    ...
    bncNetwork.buffers.forEach((clientBuffer) => {
      ...
      let existingBuffers =
          buffers.filter((bncBuffer) => bncBuffer.name.toLowerCase() ===
    ...
    this.state.networks.forEach((network) => {
      ...
      if (!snapshot.bncnetid && tags.host && tags.port && tags.nick) {
        ...
        controller.ircClient.bnc
            .addNetwork(network.name, tags.host, tags.port, tags.tls, tags.nick,
                        tags.user, tags.password)
            .then((networkInfo) => {
  ...
  listenToState() {
    ...
    state.$on("network.connecting", (event) => {
      ...
      if (network.connection.bncnetid) {
        controller.ircClient.raw("BOUNCER connect " +
