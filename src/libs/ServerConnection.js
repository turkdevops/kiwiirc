 ...
 *     Client > START             Client tells the server it's starting a
 * session Server > SESSION 1234      Server sends the client the current
 * session ID Client > :1                Client is creating a new channel, ID 1
 ...
 * @param {String} _socketChannel The optional multiplexed channel ID.
...
function createNewConnection(wsAddr, sessionId) {
  ...
  connection.reconnect = connection.connect = function connect() {
    ...
    connection.ws.onopen = () => {
      let connectStr =
          sessionId ? 'CONTROL SESSION ' + sessionId : 'CONTROL START';
    ...
    connection.ws.onmessage = (event) => {
      ...
      // If the message starts with ":channel " then extract that channel and
...
function createChannelOnConnection(connection, channelId) {
  ...
  function ConnectionChannel(options) {
    ...
    connection.on('open', () => { connection.ws.send(':' + channelId); });
    ...
    connection.on('open.' + channelId, () => {
      ...
      if (sendControlBuffer.length) {
        sendControlBuffer.forEach((line) => { channel.sendControl(line); });
        sendControlBuffer = [];
      }
    ...
    // This is not supported but irc-framework transports need it, so just noop
    // it
    ...
    channel.initChannel =
        function initChannel() { connection.ws.send(':' + channelId); };
