...
export default function chathistoryMiddleware() {
    ...
    function theMiddleware(command, event, client, next) {
        if (
            command.toLowerCase() === "batch end chathistory" &&
            client.chathistory
        ) {
...
function addFunctionsToClient(client) {
    ...
    history.before = (target, dateOrTime) =>
        new Promise((resolve) => {
            ...
            client.raw(
                ...
                "BEFORE",
                target,
                messageReference(dateOrTime),
                "50"
            );
            history.batchCallbacks.add(target, resolve);
        });

    history.after = (target, dateOrTime) =>
        new Promise((resolve) => {
            if (!history.isSupported()) {
                resolve();
                return;
    ...
    history.latest = (target, dateOrTime) =>
        new Promise((resolve) => {
            if (!history.isSupported()) {
                resolve();
                return;
    ...
    history.around = (target, dateOrTime) =>
        new Promise((resolve) => {
            if (!history.isSupported()) {
                resolve();
                return;
