...
export default function chathistoryMiddleware() {
  ...
  function theMiddleware(command, event, client, next) {
    if (command.toLowerCase() === "batch end chathistory" &&
        client.chathistory) {
      let target = event.params[0];
      client.chathistory.batchCallbacks.resolve(target, event);
    }

    if (command.toLowerCase() === "fail" &&
        event.params[0].toLowerCase() === "chathistory") {
      // FAIL CHATHISTORY MESSAGE_ERROR the_given_command #target :Messages
      // could not be ...
...
function addFunctionsToClient(client) {
  ...
  history.batchCallbacks = {
    ...
    add(target, cb) {
      this.callbacks[target.toLowerCase()] =
          this.callbacks[target.toLowerCase()] || [];
    ...
    targetCallbacks(target) { return this.callbacks[target.toLowerCase()]; },
  };
  ...
  history.before = (target, dateOrTime) => new Promise((resolve) => {
    ...
    client.raw("CHATHISTORY", "BEFORE", target, messageReference(dateOrTime),
               "50");
  ...
  history.after = (target, dateOrTime) => new Promise((resolve) => {
    ...
    client.raw("CHATHISTORY", "AFTER", target, messageReference(dateOrTime),
               "50");
    history.batchCallbacks.add(target, resolve);
  });
  ...
  history.latest = (target, dateOrTime) => new Promise((resolve) => {
    ...
    client.raw("CHATHISTORY", "LATEST", target, messageReference(dateOrTime),
               "50");
    history.batchCallbacks.add(target, resolve);
  });
  ...
  history.around = (target, dateOrTime) => new Promise((resolve) => {
    ...
    client.raw("CHATHISTORY", "AROUND", target, messageReference(dateOrTime),
               "50");
    history.batchCallbacks.add(target, resolve);
  });

  history.between = (target, fromDateOrTime, toDateOrTime) =>
      new Promise((resolve) => {
        if (!history.isSupported()) {
          resolve();
          return;
