...
const state = new Vue({
  ...
  methods : {
    ...
    importState(stateStr) {
      ...
      if (importObj && importObj.networks) {
        ...
        importObj.networks.forEach((importNetwork) => {
          let network =
              new NetworkState(importNetwork.id, state, userDict, bufferDict);
          ...
          importNetwork.buffers.forEach((impBuffer) => {
            let buffer =
                new BufferState(impBuffer.name, network.id, state, messages);
    ...
    getActiveNetwork() { return this.getNetwork(this.ui.active_network); },
    ...
    getNetworkFromAddress(netAddr) {
      return _.find(this.networks, (net) => {
        let isMatch =
            netAddr.toLowerCase() === net.connection.server.toLowerCase();
        return isMatch;
      });
    },

    getNetworkFromBncNetId(bncnetid) {
      return _.find(this.networks,
    ...
    getActiveBuffer() {
      return this.getBufferByName(this.ui.active_network,
    ...
    setActiveBuffer(networkid, bufferName) {
      if (!networkid) {
        ...
        if (this.ui.active_network) {
          ...
          this.ui.last_active_buffers =
              lastActive.splice(lastActive.length - 20);
        }
    ...
    getOrAddBufferByName(networkid, bufferName) {
      ...
      let buffer =
          _.find(network.buffers, (b) => b.name.toLowerCase() === toMatch);
    ...
    removeBuffer(buffer) {
      ...
      if (buffer.isQuery()) {
        let remainingBuffers =
            state.getBuffersWithUser(network.id, buffer.name);
    ...
    addMessage(buffer, message) {
      ...
      let isActiveBuffer = buffer.networkid === this.ui.active_network &&
      ...
      let isHighlight = !network
                            ? false
                            : Misc.mentionsNick(bufferMessage.message,
                                                network.ircClient.user.nick);

      // Check for extra custom highlight words
      let extraHighlights =
          (state.setting("highlights") || "").toLowerCase().split(" ");
      ...
      if (isNewMessage && settingAlertOn !== "never" &&
          message.type !== "nick" && message.type !== "mode" &&
          message.type !== "traffic" && !buffer.isSpecial() &&
          !bufferMessage.ignore && !isSelf) {
        let notifyTitle = "";
    ...
    removeUser(networkid, user) {
      ...
      buffers.forEach(
          (buffer) => { state.removeUserFromBuffer(buffer, user.nick); });
    ...
    removeUserFromBuffer(buffer, nick) { buffer.removeUser(nick); },
    ...
    getStartups() { return availableStartups; },
  },
});
