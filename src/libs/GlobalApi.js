...
export default class GlobalApi extends EventEmitter {
  ...
  versionMatches(v) { return compareVersions(this.version, v) >= 0; }
  ...
  setThemeManager(themeManager) { this.themes = themeManager; }

  /**
   * Change the logging level output
   * @param {number} newLevel The new logging level
   */
  logLevel(newLevel) { Logger.setLevel(newLevel); }
   ...
   */
  showInSidebar(component) { this.state.$emit("sidebar.component", component); }
