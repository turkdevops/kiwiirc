  ...
  const retHtml = blocks.reduce((html, block, i) => {
    ...
    case 'url':
      content = linkifyUrl(block, {
        ...
        handleClass :
            'fa fa-share-square u-link kiwi-messagelist-message-linkhandle',
      });
      break;
    case 'user':
    ...
    case 'emoji':
      content =
          addEmoji(block, blocks.length === 1, emojiLocation, showEmoticons);
      break;
    default:
...
function linkifyUrl(block, _opts) {
  ...
  let out =
      `<a target="_blank" href="${href}" rel="noopener noreferrer">${nice}</a>`;

  if (opts.addHandle) {
    let cssClass = opts.handleClass || '';
    let content = opts.handleContent || '';
    out += `<a data-url="${escape(block.meta.url)}" class="${cssClass}">${
        content}</a>`;
  }
...
function linkifyUser(block) {
  ...
  return `<a class="kiwi-nick" data-nick="${escaped}"` +
         (colour ? ` style="color:${colour}"` : '') + `>${escaped}</a>`;
}

function linkifyChannel(block) {
  return `<a class="u-link kiwi-channel" data-channel-name="${
      escape(block.meta.channel)}">${escape(block.content)}</a>`;
}
...
function addEmoji(block, isSingle, emojiLocation, showEmoticons) {
  ...
  const classes = 'kiwi-messagelist-emoji' +
                  (isSingle ? ' kiwi-messagelist-emoji--single' : '');
  const src = `${emojiLocation}${emoji}.png`;

  return `<img class="${classes}" src="${src}" alt="${
      escape(block.content)}" title="${escape(block.content)}" />`;
}
...
function buildSpan(content, classes, style) {
  ...
  return ('<span' + (style !== '' ? ` style="${style}"` : '') +
          (classes !== '' ? ` class="${classes}"` : '') + `>${content}</span>`);
}
