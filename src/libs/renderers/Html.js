...
function render(blocks, renderEmoticons) {
    ...
    const showEmoticons =
        typeof renderEmoticons === "undefined"
...
function linkifyUrl(block, _opts) {
    ...
    if (opts.addHandle) {
        ...
        out += `<a data-url="${escape(
            block.meta.url
        )}" class="${cssClass}">${content}</a>`;
    }
...
function linkifyChannel(block) {
    return `<a class="u-link kiwi-channel" data-channel-name="${escape(
        block.meta.channel
    )}">${escape(block.content)}</a>`;
}
...
function addEmoji(block, isSingle, emojiLocation, showEmoticons) {
    ...
    return `<img class="${classes}" src="${src}" alt="${escape(
        block.content
    )}" title="${escape(block.content)}" />`;
}
