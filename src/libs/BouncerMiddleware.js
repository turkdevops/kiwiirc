...
export default function bouncerMiddleware() {
    ...
    function theMiddleware(command, message, rawLine, client, next) {
        ...
        if (params[0] === "addnetwork" && params[2].substr(0, 4) === "ERR_") {
            ...
            client.command_handler.emit(
                "bouncer addnetwork error " + netName,
                eventObj
            );
        } else if (
            params[0] === "addnetwork" &&
...
function addFunctionsToClient(client) {
    ...
    bnc.bufferSeen = function bufferSeen(netId, bufferName, seenTime) {
        return new Promise((resolve, reject) => {
            ...
            client.raw(
                `BOUNCER changebuffer ${netId} ${bufferName} seen=${timeStr}`
            );
        });
    };

    bnc.addNetwork = function addNetwork(
        netName,
        host,
        port,
        tls,
        ...
        return new Promise((resolve, reject) => {
            ...
            function onError(event) {
                client.off(
                    "bouncer addnetwork ok " + netName.toLowerCase(),
                    onOk
                );
                reject({
                    error: event.error,
