...
export default function bouncerMiddleware() {
  ...
  function theMiddleware(command, message, rawLine, client, next) {
    ...
    if (params[0] === "listnetworks" &&
        [ "end", "RPL_OK" ].indexOf(params[1]) > -1) {
    ...
    if (params[0] === "addnetwork" && params[2].substr(0, 4) === "ERR_") {
      ...
      client.command_handler.emit("bouncer addnetwork error " + netName,
                                  eventObj);
    } else if (params[0] === "addnetwork" &&
               [ "end", "RPL_OK" ].indexOf(params[3]) > -1) {
...
function addFunctionsToClient(client) {
  ...
  bnc.getNetworks = function getNetworks() {
    return new Promise((resolve, reject) => {
      ...
      client.once("bouncer networks", (networks) => { resolve(networks); });
    });
  };
  ...
  bnc.getBuffers = function getBuffers(netId) {
    return new Promise((resolve, reject) => {
      ...
      client.once("bouncer buffers " + netId,
                  (buffers) => { resolve(buffers); });
    });
  };
  ...
  bnc.removeNetwork = function removeNetwork(netId, bufferName) {
    return new Promise(
        (resolve, reject) => { client.raw(`BOUNCER delnetwork ${netId}`); });
  };
