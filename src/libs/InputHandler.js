...
export default class InputHandler {
  ...
  processLine(rawLine, context = this.defaultContext()) {
    ...
    if (inputCommands[command.toLowerCase()]) {
      inputCommands[command.toLowerCase()].call(this, eventObj, command,
                                                params);
    }
...
inputCommands.lines = function inputCommandLines(event, command, line) {
  ...
  line.split("|").forEach((subLine) => { this.processLine(subLine.trim()); });
};
...
function handleMessage(type, event, command, line) {
  ...
  let extractedTarget =
      network.ircClient.network.extractTargetGroup(bufferName);
  if (extractedTarget) {
    localBuffer = extractedTarget.target;
  }

  let buffer = localBuffer.length &&
...
inputCommands.msg = function inputCommandMsg(
    event, command,
    line) { handleMessage.call(this, "privmsg", event, command, line); };
inputCommands.action = function inputCommandMsg(
    event, command,
    line) { handleMessage.call(this, "action", event, command, line); };
inputCommands.notice = function inputCommandMsg(
    event, command,
    line) { handleMessage.call(this, "notice", event, command, line); };
...
inputCommands.join = function inputCommandJoin(event, command, line) {
  ...
  bufferObjs.forEach((bufferObj, idx) => {
    ...
    let chanName = network.isChannelName(bufferObj.name) ? bufferObj.name
                                                         : "#" + bufferObj.name;
...
inputCommands.part = function inputCommandPart(event, command, line) {
  ...
  bufferNames.forEach(
      (bufferName) => { network.ircClient.part(bufferName, message); });
};
...
inputCommands.whois = function inputCommandWhois(event, command, line) {
  ...
  network.ircClient.whois(parts[0], parts[0], (whoisData) => {
    ...
    if (whoisData.nick && whoisData.hostname) {
      display(formats.mask.replace("{{nick}}", whoisData.nick)
    ...
    if (whoisData.actual_hostname && whoisData.actual_ip) {
      display(
          formats.from.replace("{{actual_hostname}}", whoisData.actual_hostname)
    ...
    if (whoisData.server) {
      display(formats.server.replace("{{server}}", whoisData.server)
    ...
    if (whoisData.registered_nick) {
      display(formats.registered_nick.replace("{{registered_nick}}",
    ...
    if (whoisData.idle) {
      ...
      display(formats.idle.replace("{{idle}}",
...
inputCommands.mode = function inputCommandMode(event, command, line) {
  ...
  if (parts[0]) {
    ...
    if (parts[0] === "+b" && !parts[1]) {
      ...
      setTimeout(() => { buffer.flags.requested_banlist = false; }, 4000);
    }
    ...
    if (target === buffer.name) {
      ...
      setTimeout(() => { buffer.flags.requested_modes = false; }, 4000);
    }
  }
};
...
inputCommands.list = function inputCommandList(event, command, line) {
  ...
  let network = this.state.getActiveNetwork();
  if (!network.channel_list.length &&
