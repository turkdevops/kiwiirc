...
export default class Message {
  ...
  toHtml(messageList) {
    ...
    let blocks =
        parseMessage(this.message, {extras : useExtraFormatting}, userList);

    state.$emit("message.prestyle", {message : this, blocks : blocks});

    let content = toHtml(blocks, showEmoticons);

    this.mentioned_urls = blocks.filter((block) => block.type === "url")
  ...
  maybeAutoEmbed() {
    ...
    let whitelistRegex =
        state.setting("buffers.inline_link_auto_preview_whitelist");
