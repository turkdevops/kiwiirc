...
export default class BufferState {
    ...
    isQuery() {
        ...
        return (
            chanPrefixes.indexOf(this.name[0]) === -1 &&
            !this.isSpecial() &&
            !this.isServer()
        );
    }
    ...
    isUserAnOp(nick) {
        ...
        let hasOp = _.find(
            modes,
            (mode) => opModes.indexOf(mode.toLowerCase()) > -1
        );
    ...
    requestScrollback(_direction) {
        ...
        if (direction === "backward") {
            ...
            throw new Error(
                "Invalid direction for requestScrollback(): " + _direction
            );
        }
    ...
    markAsRead(delayed) {
        ...
        if (delayed) {
            ...
            let network = this.getNetwork();
            let allowedUpdate = !network
                ? false
                : this.isChannel() || this.isQuery();
...
function createMessageBatch(bufferState) {
    ...
    let addMultipleMessages = (newMessages) => {
        ...
        if (toAdd.length > 0) {
            bufferState.messagesObj.messages = bufferState.messagesObj.messages.concat(
                toAdd
            );
            toAdd.forEach((msg) => {
    ...
    let trimMessages = () => {
        ...
        if (bufferState.messagesObj.messages.length > scrollbackSize) {
            let removed = bufferState.messagesObj.messages.splice(
                0,
                length - scrollbackSize
            );
            removed.forEach(
                (msg) => delete bufferState.messagesObj.messageIds[msg.id]
            );
        }
    };
...
function maybeStartWhoLoop(bufferState) {
    ...
    function updateWhoStatusLoop() {
        ...
        let hasAwayNotify = network.ircClient.network.cap.isEnabled(
            "away-notify"
        );
