...
export default class BufferState {
  constructor(name, networkid, state, messageDict) {
    ...
    // Counter for chathistory requests. While this value is 0, it means that
    // this buffer is still loading messages
    ...
    // This will make the `getLoadingState()` stay as 'loading' while the
    // chathistory reloads.
    ...
    if (this.isQuery() &&
        this.getNetwork().ircClient.chathistory.isSupported()) {
      // Get PM message histories, while channel buffers request it after their
      // nicklist has been received
      this.requestLatestScrollback();
    }
  }
  ...
  set topic(newVal) { this.topics.push(newVal); }

  getNetwork() { return this.state.getNetwork(this.networkid); }
  ...
  isQuery() {
    ...
    return chanPrefixes.indexOf(this.name[0]) === -1 && !this.isSpecial() &&
           !this.isServer();
  }
  ...
  isUserAnOp(nick) {
    ...
    let hasOp =
        _.find(modes, (mode) => opModes.indexOf(mode.toLowerCase()) > -1);
  ...
  rename(newName) {
    ...
    let bufferMessages =
        _.find(this.messageDict, {networkid : network.id, buffer : oldName});
  ...
  requestScrollback(_direction) {
    ...
    // Going backwards takes the earliest message we already have and requests
    // messages before it. Going forward takes the last message we have and
    // requests messages after it.
    ...
    if (direction === 'backward') {
      ...
      throw new Error('Invalid direction for requestScrollback(): ' +
                      _direction);
    }
    ...
    ircClient.chathistory[chathistoryFuncName](this.name, time)
        .then((event) => {
          ...
          // The BNC server may reply with messages that are already in the
          // buffer. This var stores whether there are new messages in the
          // chathistory response.
          let hasNewMessages = event.commands.some(
              (msg) => msg.tags.msgid &&
                       !this.messagesObj.messageIds[msg.tags.msgid]);
        ...
        .finally(() => { this.flag('is_requesting_chathistory', false); });
  }
  ...
  requestLatestScrollback() {
    ...
    this.flag('is_requesting_chathistory', true);
    this.chathistory_request_count += 1;
    ircClient.chathistory.before(this.name, '*')
        .finally(() => { this.flag('is_requesting_chathistory', false); });
  }
  ...
  markAsRead(delayed) {
    ...
    if (delayed) {
      this.active_timeout =
          setTimeout(this.markAsRead.bind(this), 10000, false);
    } else {
  ...
  addUser(user) { this.addUserBatch(user); }
  ...
  clearUsers() {
    ...
    _.each(this.users, (userObj, nick) => { delete userObj.buffers[this.id]; });

    this.state.$set(this, 'users', {});
  }

  addMessage(message) { this.addMessageBatch(message); }
  ...
  scrollToMessage(id) { this.state.$emit('messagelist.scrollto', {id : id}); }
  ...
  getLoadingState() {
    ...
    // Hack; We need to make vue aware that we depend on message_count in order
    // to update the loading state. eslint-disable-next-line no-unused-vars
    ...
    if (networkState === 'disconnected') {
      ...
      return 'connecting';
    } else if (networkState === 'connected' && this.joined && this.enabled &&
               (historySupport &&
                (this.flags.is_requesting_chathistory ||
                 // If chathistory is supported then a request will always be
                 // made when first joining a channel. If request_count===0 then
  ...
  isReady() { return this.getLoadingState() === 'done'; }
}
...
function createUserBatch(bufferState) {
  ...
  let addMultipleUsers = (users) => {
    ...
    users.forEach((u) => { o[u.nick.toLowerCase()] = u; });
    bufferState.users = o;
  };
...
function createMessageBatch(bufferState) {
  ...
  let addMultipleMessages = (newMessages) => {
    let toAdd = newMessages.filter(
        (msg) => !bufferState.messagesObj.messageIds[msg.id]);
    if (toAdd.length > 0) {
      bufferState.messagesObj.messages =
          bufferState.messagesObj.messages.concat(toAdd);
      toAdd.forEach(
          (msg) => { bufferState.messagesObj.messageIds[msg.id] = msg; });
      trimMessages();
    }
    // Trigger Vue's reactivity on the buffer whether messages were added or
    // not, just in case anything was depending on the batch queue which has now
    // been emptied.
    bufferState.message_count++;
  };
  let trimMessages = () => {
    ...
    if (bufferState.messagesObj.messages.length > scrollbackSize) {
      ...
      removed.forEach((msg) =>
                          delete bufferState.messagesObj.messageIds[msg.id]);
    }
  };
...
// Update our user list status every 30seconds to get each users current away
// status
function maybeStartWhoLoop(bufferState) {
  ...
  function nextLoop() { setTimeout(updateWhoStatusLoop, 30000); }
  ...
  function updateWhoStatusLoop() {
    ...
    if (whoLoop && networkConnected && isJoined && !hasAwayNotify) {
      network.ircClient.who(bufferState.name, () => { nextLoop(); });
    } else {
      nextLoop();
    }
  }
