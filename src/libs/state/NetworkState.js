...
export default class NetworkState {
  constructor(id, appState, userDict, bufferDict) {
    ...
    def(this, 'users', Object.create(null),
        (newVal) => { appState.$set(userDict.networks, this.id, newVal); });
  ...
  get buffers() { return this.bufferDict.networks[this.id]; }

  connect(...args) { this.ircClient.connect(...args); }

  bufferByName(name) { return this.appState.getBufferByName(this.id, name); }

  serverBuffer() { return this.appState.getBufferByName(this.id, '*'); }
  ...
  currentUser() { return this.appState.getUser(this.id, this.nick); }

  userByName(nick) { return this.appState.getUser(this.id, nick); }
}
