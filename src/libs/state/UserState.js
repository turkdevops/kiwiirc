...
export default class UserState {
  constructor(user) {
    ...
    // Whois details are non-enumerable properties (vues $watch won't cover
    // these properties)
  ...
  isAway() { return !!this.away; }

  typingStatus(target, status) {
    if (!this.typingState[target.toLowerCase()]) {
      Vue.set(this.typingState, target.toLowerCase(), {
        started : 0,
        status : "",
      });
    }
    ...
    // Paused state gets a longer timeout as it's usually someone stopping
    // typing to think about their words
    let timeoutLen = status === "paused" ? 30000 : 6000;

    typing.timeout =
        setTimeout(() => this.typingStatus(target, "done"), timeoutLen);
